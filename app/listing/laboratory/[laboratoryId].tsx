...
// Types
import { ListingTypeLaboratory } from '@/types/types';

// Data
import listingData from '@/data/laboratorios.json';

// Dimensions
const { width } = Dimensions.get('window');
const IMG_HEIGHT = 300;

const ListingLaboratoriesDetails = () => {
    const { laboratoryId } = useLocalSearchParams();

    const laboratoryIdNumber = Array.isArray(laboratoryId) ? parseInt(laboratoryId[0]) : parseInt(laboratoryId);

    const listing = (listingData as ListingTypeLaboratory[]).find((item) => item.id === laboratoryIdNumber);
...
export default ListingLaboratoriesDetails;
