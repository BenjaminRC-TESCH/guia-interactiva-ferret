...
// Data
import listingData from '@/data/auditorios.json';

// Dimensions
const { width } = Dimensions.get('window');
const IMG_HEIGHT = 300;

const ListingAuditoriumDetails = () => {
    const { auditoriumId } = useLocalSearchParams();

    const auditoriumIdNumber = Array.isArray(auditoriumId) ? parseInt(auditoriumId[0]) : parseInt(auditoriumId);

    const listing = (listingData as ListingTypeAuditorium[]).find((item) => item.id === auditoriumIdNumber);
                        ...
                        <View>
                            ...
                            <View>
                                {listing.links?.map((link, index) => (
                                    <View style={styles.functionItem} key={index}>
                                        <Octicons style={styles.icon} name='dot-fill' size={12} color='black' />
                                        <Text style={styles.functionsText}>{link}</Text>
                                    </View>
                                ))}
                            </View>
                            {/* POSTDESCRIPTION */}
...
export default ListingAuditoriumDetails;
