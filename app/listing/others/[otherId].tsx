...
// Types
import { ListingTypeOther } from '@/types/types';

// Data
import listingData from '@/data/otros.json';

// Dimensions
const { width } = Dimensions.get('window');
const IMG_HEIGHT = 300;

const ListingOtherDetails = () => {
    const { otherId } = useLocalSearchParams();

    const otherIdNumber = Array.isArray(otherId) ? parseInt(otherId[0]) : parseInt(otherId);

    const listing = (listingData as ListingTypeOther[]).find((item) => item.id === otherIdNumber);
...
export default ListingOtherDetails;
